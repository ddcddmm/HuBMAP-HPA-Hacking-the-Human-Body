...
from model_coat_daformer_tuer import *
from dataset import *

import torch.cuda.amp as amp
is_amp = True  #True #False
exp_name = 'coat0915_dynamic_1m1_tuer'
FOLD = 0
BATCH_SIZE = 16
LR = 1e-4
# DEVICE_IDS = [0, 1]
initial_checkpoint = '/home/vipuser/workshop/pretrain/coat_small_7479cf9b.pth'
	...
	for t, batch in enumerate(valid_loader):
		...
		with torch.no_grad():
			with amp.autocast(enabled = is_amp):
				...
				output = net(batch)
				# output = data_parallel(net, batch, device_ids=[0, 1]) #net(input)#
...
def run_train():
	...
	out_dir = f'./result/coat/{exp_name}/fold-%d' % (fold)
	...
	train_dataset = HubmapDataset_dynamic(train_df, train_augment5b)
	...
	if initial_checkpoint is not None:
		state_dict = torch.load(initial_checkpoint, map_location=lambda storage, loc: storage)['model']
		...
		for t, batch in enumerate(train_loader):
			...
			if 1:
				with amp.autocast(enabled = is_amp):
					output = net(batch)
					loss0  = output['bce_loss'].mean()
					loss_0  = output['aux0_loss'].mean()
					loss_1  = output['aux1_loss'].mean()
					loss_2  = output['aux2_loss'].mean()
					loss_3  = output['aux3_loss'].mean()
					loss1  = output['tuer_loss'].mean()
			
				optimizer.zero_grad()
				scaler.scale(loss0+0.1*loss_0+0.2*loss_1+0.2*loss_2+0.2*loss_3+0.4*loss1).backward()
			...
			# print statistics  --------
			batch_loss[:5] = [loss0.item(),loss_0.item(),loss_1.item(),loss_2.item(),loss_3.item()]
