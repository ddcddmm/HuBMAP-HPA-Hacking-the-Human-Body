...
class CoaT(nn.Module):
	""" CoaT class. """
	def __init__(self, patch_size=16, in_chans=3, embed_dims=[0, 0, 0, 0, 0],
		...
		if self.parallel_depth > 0:
			self.parallel_blocks = nn.ModuleList([
				ParallelBlock(
					...
					drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr, norm_layer=norm_layer,
					shared_cpes=nn.ModuleList([self.cpe1, self.cpe2, self.cpe3, self.cpe4, self.cpe5]),
					shared_crpes=nn.ModuleList([self.crpe1, self.crpe2, self.crpe3, self.crpe4, self.crpe5])
				)
